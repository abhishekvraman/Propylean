...
def test_Length_incorrect_type_to_value():
    ...
    assert "Incorrect type 'list' provided to 'value'. Can be any one from '('int', 'float')" in str(exp)
    with pytest.raises(Exception) as exp:
        l = properties.Length()
        l.value = []
    assert "Incorrect type 'list' provided to 'value'. Can be any one from '('int', 'float')" in str(exp)
...
#         l.time_series = []
#     assert "Incorrect type 'list' provided to 'time_series'. Can be any one from '('Series', 'DataFrame', 'dict')" in str(exp)
